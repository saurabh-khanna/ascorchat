...
2. **Participant routing**: In **survey mode** (`N_CONDITIONS = 1`), no routing logic is applied; all participants are directed without passcode to the same chat interface. In **experiment mode** (`N_CONDITIONS >= 2`), participant allocation is governed by one of two sub-modes. In *passcode routing* (activated when every condition specifies a `passcode`), participants are first presented with a passcode entry interface; the submitted passcode is resolved in a case-insensitive mapping to determine the experimental condition. This mapping is deterministic across page reloads, such that a given passcode consistently maps to the same condition without reliance on server-side session storage. In *random routing* (used when no passcodes are defined but `N_CONDITIONS >= 2`), an experimental condition is sampled from a uniform distribution upon the initial page load.

3. **Chat interaction**: Participants engage in dialogue with the chatbot via a minimalistic user interface developed using Streamlit [@streamlit]. The persona definition is programmatically inserted as a system-level prompt at index 0 of every API request and remains hidden from the user interface at all times [@ma2021one]. Model outputs are delivered using token‑wise streaming to approximate a natural, turn‑taking conversational dynamic. To mitigate inadvertent termination of the interaction, we implement a two‑stage *End chat → Confirm* control scheme. These design decisions - specifically, response streaming and explicit end‑of‑session confirmation - are informed by prior research on sustaining user engagement with relational agents in repeated, survey‑like interaction contexts [@bickmore2010maintaining].
...
5. **Data ingestion**: Researchers find this transcript data included when they export their survey data from the parent platform (e.g. Qualtrics). The structured JSON format allows researchers to easily process the transcript by parsing the JSON structure using standard libraries, for example in Python:
...
During system initialization, a series of configuration validation procedures is executed to ensure the integrity of the application setup and to preemptively identify common configuration errors. These procedures detect missing API credentials, inconsistencies in `N_CONDITIONS` declarations, partially specified condition lists, redundant passcode assignments, and empty passcode fields. Any such issues generate explicit, actionable error messages prior to the rendering of any participant-facing user interface components.

![Passcode entry interface in experimental mode. Participants input a condition-specific code supplied by the survey platform, which programmatically routes them to their assigned chatbot experimental condition.\label{fig:entry}](surveychat-interface-1.png){ width=100% }

![The chat interface presented during an active conversation. Participant messages and incrementally generated assistant responses are rendered in a threaded layout, accompanied by an *End chat* control that enables explicit termination of the session.\label{fig:chat}](surveychat-interface-2.png){ width=100% }

The passcode routing mechanism is implemented to align with mainstream survey workflows like Qualtrics. In this setup, participants are randomly assigned to experimental conditions within Qualtrics. A condition-specific code is then displayed on a survey page, and participants are directed to the `surveychat` URL via a link or an embedded iFrame. Since the chatbot can be embedded directly within a Qualtrics survey page, it allows participants to interact with it without exiting the survey interface. Upon completion of the chatbot interaction, participants copy the resulting JSON transcript into a Qualtrics text-entry question, thereby consolidating treatment assignment and outcome data within a single data export.
...
Emerging empirical evidence comparing conversational interviewers based on large language models (LLMs) with human interviewers suggests that AI-administered interviews can produce data of comparable quality coupled with potential for greater scalability [@wuttke2025ai]. `surveychat` has been developed to support ongoing research programs in survey methodology and experimental social science that employ LLM-based conversational agents as research instruments. The project is designed for reuse across a broad range of social science domains in which controlled conversational experiments or chatbot-mediated structured interviews are increasingly becoming essential. The application's open-source nature is intended to facilitate community-driven extensions and adaptations, for example, to support additional API providers, implement alternative participant routing schemes, or enable integration with more survey platforms. The modular system design allows researchers to modify the underlying codebase to address specific research questions or methodological requirements while preserving a standardised interface for participant interaction and data collection.

The project aims to serve as a first methodological step towards the development of LLM-based research instruments, highlighting a design that balances experimental control, research participant experience, and data quality considerations in the context of conversational agents used in social science research. By providing a robust, flexible, and user-friendly platform for LLM-based conversational surveys and experiments, `surveychat` seeks to catalyse further empirical research on the design, implementation, and evaluation of AI-mediated survey instruments, thus contributing to the advancement of survey methodology in the era of large language models.
